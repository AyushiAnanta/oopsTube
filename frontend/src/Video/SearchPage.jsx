...
const SearchPage = () => {
    ...
    const fetchVideos = useCallback(async (pageNum = 1, isNewSearch = false) => {
        if (!query.trim()) return;
        
        try {
            setIsLoading(true);
            const res = await axiosInstance.get(`/videos/video?query=${encodeURIComponent(query)}&page=${pageNum}&limit=12`);
    ...
    return (
        <div className="w-full max-w-[1600px] mx-auto animate-fade-in">
            <h1 className="text-2xl font-bold text-white mb-8 flex items-center gap-3">
                ...
                Search results for <span className="text-brand-300">"{query}"</span>
            </h1>
            ...
            {videos.length === 0 && !isLoading ? (
                <div className='w-full py-20 bg-dark-800/30 rounded-2xl border border-dark-700/50 flex flex-col items-center justify-center text-center'>
                    ...
                    <p className="text-gray-500 max-w-md">Try adjusting your search terms to find what you're looking for.</p>
                </div>
            ) : (
